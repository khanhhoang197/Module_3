...
select * 
from hoadon 
where ngaymuahang between '2007-01-01' and '2007-01-02';


-- 7. In ra các số hóa đơn, trị giá hóa đơn trong tháng 1/2007, sắp xếp theo ngày (tăng dần) và trị giá của óa đơn (giảm dần).
select * 
from hoadon 
where ngaymuahang like '2007-01%' order by ngaymuahang;
select * 
from hoadon 
where ngaymuahang like '2007-01%' order by ngaymuahang desc;
...
SELECT hd.SOHOADON, hd.TRIGIA, nv.HoTen, hd.NgayMuaHang 
FROM HOADON hd
...
SELECT COUNT(MASP) AS "TỔNG SỐ SP" 
FROM SANPHAM
WHERE NUOCSX = "Trung Quoc";


-- 30. Tính tổng số sản phẩm của từng nước sản xuất.
SELECT COUNT(MASP) AS "TỔNG SỐ SP", NUOCSX  
FROM SANPHAM
GROUP BY NUOCSX;
...
JOIN CTHD CT
ON CT.MASP = SP.MASP
WHERE CT.SOLUONG = (SELECT MIN(SOLUONG) 
					FROM CTHD CT JOIN HOADON HD ON CT.SOHD = HD.SOHOADON 
                    WHERE HD.NGAYMUAHANG LIKE "2006%");
...
SELECT HD.MAKH, KH.HOTEN, COUNT(HD.MAKH) 
FROM HOADON HD 
